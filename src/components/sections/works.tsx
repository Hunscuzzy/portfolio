    ...
    >
      ...
      <div className='relative container mx-auto flex flex-col md:flex-row-reverse justify-between gap-16'>
        <div className='md:sticky top-8 h-fit  md:text-right'>
          ...
          <div className='flex justify-end gap-2 text-lg flex-wrap'>
            <SiTypescript />
            <SiJest />
            <SiCypress />
            <SiReact />
        ...
        <div className='relative border-s border-gray-200 dark:border-gray-700 space-y-4'>
          {works.map((work) => (
            <Disclosure key={work.title}>
              {({ open }) => (
                <>
                  <Disclosure.Button className='group ml-8 text-left w-full'>
                    <span className='absolute flex items-center justify-center w-6 h-6 bg-white group-hover:bg-slate-200 rounded-full -start-3 ring-7 ring-white dark:ring-blue-900 dark:bg-blue-900  dark:group-hover:bg-blue-700'>
                      {open ? (
                    ...
                    <h3 className='flex items-center mb-1 text-xl font-medium text-blue-950 dark:text-white'>
                      {work.title}
                    </h3>
                  </Disclosure.Button>
